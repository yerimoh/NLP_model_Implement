...
def create_contexts_target(data, label, content):
    ...
    for line in tqdm(data[content]):
        ...
        for i in range(len(line)):
            if line[i] in w2id.keys():
                cs.append(w2id[line[i]])
            if i > 0:
                tmp = line[i-1] + " " + line[i]
                if tmp in w2id.keys():
                    cs.append(w2id[tmp])
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(len(data[content]))):
        text = clean_str(data[content][i]).split()
        for j in range(len(text)):
            words.append(text[j])
            if j>0:
                words.append(text[j-1] + " " + text[j])
    ...
    if sys.argv[1] == "ag":
        with open("./preprocessed_data/ag_bigram.pkl", "wb") as f:
            pickle.dump([w2id, id2w, corpus_cnt, contexts, targets, test_contexts, test_targets], f, -1)
    elif sys.argv[1] == "amzf":
        with open("./preprocessed_data/amzf_bigram.pkl", "wb") as f:
            pickle.dump([w2id, id2w, corpus_cnt, contexts, targets, test_contexts, test_targets], f, -1)
    elif sys.argv[1] == "amzp":
        with open("./preprocessed_data/amzp_bigram.pkl", "wb") as f:
            pickle.dump([w2id, id2w, corpus_cnt, contexts, targets, test_contexts, test_targets], f, -1)
    elif sys.argv[1] == "dbp":
        with open("./preprocessed_data/dbp_bigram.pkl", "wb") as f:
            pickle.dump([w2id, id2w, corpus_cnt, contexts, targets, test_contexts, test_targets], f, -1)
    elif sys.argv[1] == "sogou":
        with open("./preprocessed_data/sogou_bigram.pkl", "wb") as f:
            pickle.dump([w2id, id2w, corpus_cnt, contexts, targets, test_contexts, test_targets], f, -1)
    elif sys.argv[1] == "yaha":
        with open("./preprocessed_data/yaha_bigram.pkl", "wb") as f:
            pickle.dump([w2id, id2w, corpus_cnt, contexts, targets, test_contexts, test_targets], f, -1)
    elif sys.argv[1] == "yelpf":
        with open("./preprocessed_data/yelp_bigram.pkl", "wb") as f:
            pickle.dump([w2id, id2w, corpus_cnt, contexts, targets, test_contexts, test_targets], f, -1)
    elif sys.argv[1] == "yelpp":
        with open("./preprocessed_data/yelpp_bigram.pkl", "wb") as f:
            pickle.dump([w2id, id2w, corpus_cnt, contexts, targets, test_contexts, test_targets], f, -1)